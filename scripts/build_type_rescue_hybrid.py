...
from vehicle_damage.model import Dinov2FourLayerTypeRescueDamageSegmenter
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a four-layer model with one gated parent type rescue"
    )
    parser.add_argument("--parent", required=True)
    parser.add_argument("--four-layer", required=True)
    parser.add_argument("--rescue-type-index", type=int, required=True)
    ...
    model = Dinov2FourLayerTypeRescueDamageSegmenter(
        ...
        rescue_type_index=args.rescue_type_index,
        rescue_threshold=args.rescue_threshold,
    )
    state = model.state_dict()
    for name in state:
        if name.startswith("rescue_presence_head."):
            state[name] = parent["model"][name.replace("rescue_", "", 1)]
        elif name.startswith("rescue_type_head."):
            state[name] = parent["model"][name.replace("rescue_", "", 1)]
        elif name.startswith("rescue_exterior_head."):
            state[name] = parent["model"][name.replace("rescue_", "", 1)]
    ...
    result["architecture"] = "dinov2_4layer_type_rescue"
    result["config"] = {
        **config,
        "architecture": "dinov2_4layer_type_rescue",
        "rescue_type_index": args.rescue_type_index,
        "rescue_threshold": args.rescue_threshold,
    }
    result["type_rescue_hybrid"] = {
        "method": "parent_high_confidence_type_union",
        "rescue_type_index": args.rescue_type_index,
        "rescue_threshold": args.rescue_threshold,
